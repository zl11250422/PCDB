...
class RFDB(nn.Module):
    def __init__(self, in_channels, distillation_rate=0.25):
        ...
        mlp_layer1: List[nn.Module] = [
            nn.Conv2d(self.rc, self.rc, 1, bias=False),
            nn.BatchNorm2d(self.rc),
            nn.ReLU(),
            nn.Conv2d(self.rc, self.rc, 1, bias=False)
        ]
        self.mlp1 = nn.Sequential(*mlp_layer1)

        mlp_layer2: List[nn.Module] = [
            nn.Conv2d(self.rc, self.rc, 1, bias=False),
            nn.BatchNorm2d(self.rc),
            nn.ReLU(),
            nn.Conv2d(self.rc, self.rc, 1, bias=False)
        ]
        self.mlp2 = nn.Sequential(*mlp_layer2)
        
        mlp_layer3: List[nn.Module] = [
            nn.Conv2d(self.rc, self.rc, 1, bias=False),
            nn.BatchNorm2d(self.rc),
            nn.ReLU(),
            nn.Conv2d(self.rc, self.rc, 1, bias=False)
        ]
        self.mlp3 = nn.Sequential(*mlp_layer3)        
